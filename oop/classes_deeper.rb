...
# begin
p 1.class #=> Integer
# Viking class will inhert from the class
class Person
  def initialize(name, age, strength, _dead)
    @name = name
    @age = age
    @strength = strength
    @dead = false
...
class Viking < Person
  ...
  def self.create_warrior(name)
    ...
    weapon = %w[Sword Axe Club].sample
    dead = false
    Viking.new(name, age, strength, dead, weapon)
  end

  # useful to make random warrior
  def self.random_name
    %w[loki Seger Lydia].sample #=> sample picks random element
  end

  #=> less common way to call class to itself
  class << self
